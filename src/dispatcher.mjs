...
function engineExecutable(identity, env, platform) {
...
function repositoryIdentity(repo) {
  try {
    ...
    return { engineVersion: config.engineVersion, engineCommitSha: config.engineCommitSha };
  } catch (error) {
...
export function dispatch({ cwd = process.cwd(), argv = [], env = process.env, platform = process.platform, spawn = spawnSync } = {}) {
  try {
    ...
    const forwarded = argv[0] === "pre-push" ? ["check", ...argv.slice(1)] : argv;
    let identity;
